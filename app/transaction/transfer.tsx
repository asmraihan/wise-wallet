...
export default function TransferScreen() {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      amount: 0,
      from_account: { label: 'Select an account', value: 0 },
      to_account: { label: 'Select an account', value: 0 },
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const res = await transactionDb.transfer({
      amount: values.amount,
      from_account: values.from_account.value,
      to_account: values.to_account.value,
      date: values.date,
  });
    ...
    >
      <Form {...form}>
        ...
        <FormField
          ...
          />
          <ArrowRight size={24} className='text-white mt-8'/>
            <FormField
            control={form.control}
            name='to_account'
            render={({ field }) => (
